...
mod tests {
    ...
    #[rstest]
    #[case("send message to another user", 200, &VALID_PUBKEY, "Hello, Bob!", 14, Utc::now().timestamp() as u64, None, None, Ok(()))]
    #[case("rejected for empty message", 200, &VALID_PUBKEY, "", 14, Utc::now().timestamp() as u64, None, None, Err(SendMessageUseCaseError::EmptyMessage))]
    #[case("rejected for message has only spaces", 200, &VALID_PUBKEY, "   ", 14, Utc::now().timestamp() as u64, None, None, Err(SendMessageUseCaseError::EmptyMessage))]
    #[case("rejected for message is too long", 8, &VALID_PUBKEY, "Hello, Bob", 14, Utc::now().timestamp() as u64, None, None, Err(SendMessageUseCaseError::MessageTooLong))]
    #[case("rejected for message's kind not found", 200, &VALID_PUBKEY, "Hello, Bob", 5000, Utc::now().timestamp() as u64, None, None, Err(SendMessageUseCaseError::KindError(ValidateKindError::InvalidKindValue(5000))))]
    #[case("rejected for timestamp is invalid", 200, &VALID_PUBKEY, "Hello, Bob", 14, u64::MAX, None, None, Err(SendMessageUseCaseError::TimestampError(ValidateTimestampError::InvalidTimestamp)))]
    #[case("rejected for timestamp is far in the future", 200, &VALID_PUBKEY, "Hello, Bob", 14, (Utc::now() + Duration::seconds(60 * 10)).timestamp() as u64, None, None, Err(SendMessageUseCaseError::TimestampError(ValidateTimestampError::TimestampTooFarInTheFuture)))]
