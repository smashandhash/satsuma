...
mod tests {
    ...
    fn can_add_message_to_conversation() {
        ...
        let message = Message::new("generateId".to_string(), "npub101".to_string(), "Hello!".to_string(), Utc::now().timestamp() as u64, MessageKind::Direct("npub202".to_string()));
    ...
    fn message_should_in_order() {
        ...
        let first_message = Message::new("generateId".to_string(), "npub101".to_string(), "First".to_string(), Utc::now().timestamp() as u64, MessageKind::Direct("npub202".to_string()));
        let second_message = Message::new("generateId".to_string(), "npub202".to_string(), "Second".to_string(), Utc::now().timestamp() as u64, MessageKind::Direct("npub202".to_string()));
    ...
    fn reject_message_from_non_participant_conversation() {
        let mut conversation = Conversation::new(1, "npub101", vec!["npub101", "npub202"]);
        let outsider_message = Message::new("generateId".to_string(), "npub303".to_string(), "Hi, can I join?".to_string(), Utc::now().timestamp() as u64, MessageKind::Direct("npub202".to_string()));
